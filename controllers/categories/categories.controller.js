...
const deleteCategoryByAdmin = async (req, res) =>
{
    try
    {
        ...
        let result = await Category.findByIdAndDelete({ _id: req.params.categoryId }, { new: true })
...
const getAllParentToSubCategories = async (req, res) =>
{
    try
    {
        let categories = await Category.find({category_type:"secondary", parent_category: req.params.categoryId });
        if (!categories.length > 0)
        {
            return res.status(400).json({
                error: true,
